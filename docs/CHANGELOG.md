...
### Added
- **Major Rebranding**: Consolidated the document anonymizer as "Doc Anonymizer Pro" v1.0.0.
- **Excel-as-Source restoration**: Replaced the document-local sidecar JSON system with a direct restoration from the mapping Excel file, simplifying the document package.
- **GUI Multi-step restoration**: New intuitive workflow in the GUI for restoring documents with step-by-step file selection.

### Changed
- **Project Structure**: Performed a major cleanup and reorganization of the repository following best practices (separated logic, docs, scripts, and assets).
- **Core Replacer**: Synchronized `docx` and `pdf` restoration logic to use the same mapping engine.

### Removed
- Removed legacy sidecar generation and `load_reversal_sidecar` logic from `replacer.py`.
